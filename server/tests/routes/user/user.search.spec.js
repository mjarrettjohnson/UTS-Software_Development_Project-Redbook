...
describe(`GET ${ENDPOINT}`, () => {
  ...
  describe('#200', () => {
    it('should return an array of users', done => {
      server.get(ENDPOINT).set('Authorization', admin.generateToken()).end((err, res) => {
  ...
  describe('#403', () => {
    it('should return unauthorized if a non-admin tries to access all users', done => {
      server.get(ENDPOINT).set('Authorization', client.generateToken()).end((err, res) => {
        expect(res).to.have.status(403);
        expect(res.body.message).to.be.defined;
        expect(res.body.message).to.equal('You do not have the correct permissions to access this page.');
        done();
      });
    });
  });
